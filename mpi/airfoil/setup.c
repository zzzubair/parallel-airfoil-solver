...
void problem_set_up() {
    ...
    if (rank == 0 || rank == 1) {
        printf("Rank %d: fluid_cells=%d\n", rank, fluid_cells);
        // Check flag array near airfoil
        printf("Rank %d: flag[128][124]=%d, flag[128][125]=%d, flag[129][125]=%d, flag[130][125]=%d\n",
               rank, (int)flag[128][124], (int)flag[128][125], (int)flag[129][125], (int)flag[130][125]);
        fflush(stdout);
    }

	// NOTE: apply_boundary_conditions() is called after halo exchange in main()
	// Calling it here would access uninitialized halo data in MPI version
}