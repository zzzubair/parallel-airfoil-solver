...
void compute_tentative_velocity() {
    #pragma omp parallel for schedule(static) collapse(2) default(none) \
        shared(imax, jmax, u, v, f, flag, delx, dely, del_t, Re, y)
    ...
    #pragma omp parallel for schedule(static) collapse(2) default(none) \
        shared(imax, jmax, u, v, g, flag, delx, dely, del_t, Re, y)
...
void compute_rhs() {
    #pragma omp parallel for schedule(static) collapse(2) default(none) \
        shared(imax, jmax, f, g, rhs, flag, delx, dely, del_t)
    for (int i = 1; i < imax+1; i++) {
        for (int j = 1;j < jmax+1; j++) {
            if (flag[i][j] & C_F) {
...
double poisson() {
    ...
    #pragma omp parallel for schedule(static) collapse(2) reduction(+:p0) \
        default(none) shared(imax, jmax, p, flag)
    for (int i = 1; i < imax+1; i++) {
        for (int j = 1; j < jmax+1; j++) {
    ...
    for (iter = 0; iter < itermax; iter++) {
        for (int rb = 0; rb < 2; rb++) {
            #pragma omp parallel for schedule(static) collapse(2) \
                default(none) shared(imax, jmax, rb, p, flag, rhs, rdx2, rdy2, omega, beta_2)
            for (int i = 1; i < imax+1; i++) {
                for (int j = 1; j < jmax+1; j++) {
        ...
        res = 0.0;
        #pragma omp parallel for schedule(static) collapse(2) reduction(+:res) \
            default(none) shared(imax, jmax, p, flag, rhs, rdx2, rdy2, fluid_cells, p0)
        for (int i = 1; i < imax+1; i++) {
            for (int j = 1; j < jmax+1; j++) {
                if (flag[i][j] & C_F) {
                    double eps_E = ((flag[i+1][j] & C_F) ? 1.0 : 0.0);
...
void update_velocity() {
    #pragma omp parallel for schedule(static) collapse(2) default(none) \
        shared(imax, jmax, u, f, p, flag, del_t, delx)
    ...
    #pragma omp parallel for schedule(static) collapse(2) default(none) \
        shared(imax, jmax, v, g, p, flag, del_t, dely)
    for (int i = 1; i < imax-1; i++) {
...
void set_timestep_interval() {
    ...
    if (tau >= 1.0e-10) { /* else no time stepsize control */
        ...
        #pragma omp parallel for schedule(static) collapse(2) reduction(max:umax) \
            default(none) shared(imax, jmax, u)
        ...
        #pragma omp parallel for schedule(static) collapse(2) reduction(max:vmax) \
            default(none) shared(imax, jmax, v)
        for (int i = 1; i < imax+2; i++) {
